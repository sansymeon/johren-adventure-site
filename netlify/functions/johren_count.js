export async function handler(event) {
  ...
  if (!merchant || !token) {
    return { statusCode: 400, body: "Missing merchant/token" };
  }
...
if (hasRef && !okRef) return { statusCode: 403, body: "Forbidden" };
  ...
  const setnx = await fetch(
    `${url}/set/${encodeURIComponent(dedupeKey)}/1?nx=1`,
    { headers: auth }
  ...
  await fetch(`${url}/expire/${encodeURIComponent(dedupeKey)}/172800`, {
    headers: auth
  }).catch(() => {});

  // increment total
  await fetch(`${url}/incr/${encodeURIComponent(totalKey)}`, {
    headers: auth
  }).catch(() => {});
